...
class fake_cloud():
	...
	def form(self, density, radius, traj):

		self.rho_med = density
		self.Mdot_BHL = 0.0
		self.R = radius
		self.R0 = radius
		self.tau_ff0 = self.tau_ff()
		...
		self.kappa = traj.grid.kappa
		self.lnrho_var0  =variance_k(1./self.R0,  cs=self.cs, kappa=self.kappa, Espect = self.Espect)
		self.lnrho_med0  = np.log(density)

		self.rho_med_crit = density
		self.calc_Mgas()
		self.vol = self.M/self.rho_med
	...
	def evolve(self, t, rejuvinate=False):
		if (rejuvinate or not self.dispersed) and t-self.t>0.0:
			if self.formed and not self.dispersed:
				
				#self.accretion_step(dt, traj)
				self.SF_step(t)
				self.collapse_step(t)
				if np.isnan(self.M):
					raise Warning('Cloud mass is NaN in evolve timestep')
				self.check_dispersal()
